...
export function Header({ header }: { header: HeaderType }) {
  ...
  const isJoyflixHeader = header.brand?.title === 'JoyFlix AI';
              ...
              return (
                <NavigationMenuLink key={idx} asChild>
                  <Link
                    ...
                    className={cn(
                      'flex flex-row items-center gap-2 px-4 py-1.5 text-sm',
                      item.is_active || pathname.endsWith(item.url as string)
                        ? 'bg-muted/40 text-muted-foreground'
                        : '',
                      isJoyflixHeader &&
                        'px-2.5 py-1 text-[0.95rem] font-medium text-white/68 transition-colors hover:text-white/88',
                      item.className
                    )}
                  >
              ...
              >
                {item.children && item.children.length > 0 ? (
                  ...
                  <Link
                    ...
                    onClick={closeMenu}
                    className={cn(
                      'data-[state=open]:bg-muted flex items-center justify-between px-4 py-3 text-lg **:!font-normal',
                      item.className
                    )}
                  >
                    {item.title}
