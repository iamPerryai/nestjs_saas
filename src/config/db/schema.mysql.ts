...
export const job = table(
  ...
  {
    id: varchar191('id').primaryKey(),
    source: varchar191('source').notNull(),
    sourceJobId: varchar191('source_job_id').notNull(),
    sourceKey: varchar191('source_key').unique().notNull(),
    sourceAts: varchar191('source_ats'),
    title: varchar('title', { length: 255 }).notNull(),
    companyName: varchar('company_name', { length: 255 }).notNull().default(''),
    companyDomain: varchar191('company_domain'),
    ...
    rawPayload: longtext('raw_payload'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
    deletedAt: timestamp('deleted_at'),
  },
  (table) => [
    index('idx_job_status_posted_at').on(table.status, table.postedAt),
    index('idx_job_remote_posted_at').on(table.isRemote, table.postedAt),
    index('idx_job_company_name').on(table.companyName),
  ]
);

export const savedJob = table(
  'saved_job',
  {
    id: varchar191('id').primaryKey(),
    userId: varchar191('user_id')
  ...
  (table) => [
    index('idx_saved_job_user_created_at').on(table.userId, table.createdAt),
    index('idx_saved_job_job_id').on(table.jobId),
  ]
);

export const jobSyncRun = table(
  'job_sync_run',
  {
    id: varchar191('id').primaryKey(),
    provider: varchar191('provider').notNull(),
    status: varchar('status', { length: 50 }).notNull(),
    querySnapshot: longtext('query_snapshot'),
  ...
  (table) => [
    index('idx_job_sync_run_provider_started_at').on(
      table.provider,
      table.startedAt
    ),
  ]
